...
function toggleCard(day, checked, card) {
  state[day] = checked;
  saveState(state);
  if (checked) {
  ...
  if (txt) txt.textContent = checked ? 'PUBLICADO' : 'MARCAR COMO PUBLICADO';
  updateStats();
}
...
function renderPosts() {
  ...
  weeks.forEach(week => {
    ...
    week.days.forEach(dayNum => {
      ...
      card.innerHTML = `
        ...
        <div class="card-footer">
          <label class="check-label">
            <input type="checkbox" ${isDone ? 'checked' : ''} data-day="${dayNum}">
      ...
      // Attach event after innerHTML is set
      grid.appendChild(card);
      const cb = card.querySelector('input[type="checkbox"]');
      cb.addEventListener('change', function() {
        toggleCard(dayNum, this.checked, card);
      });
    });
